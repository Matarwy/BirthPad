...
const createProjectSchema = z
  ...
  .superRefine((value, ctx) => {
    if (Number(value.softCap) > Number(value.hardCap)) {
      ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'softCap must be <= hardCap', path: ['softCap'] });
    }
    ...
    if (value.teamVesting.enabled && value.teamVesting.durationSeconds <= 0) {
      ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'team vesting duration must be > 0', path: ['teamVesting', 'durationSeconds'] });
    }

    if (value.liquidityLock.enabled && !value.liquidityLock.lockUntil) {
      ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'lockUntil is required when liquidity lock is enabled', path: ['liquidityLock', 'lockUntil'] });
    }
  });
